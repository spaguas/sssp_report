...
def gerar_mapa_chuva(url, titulo, excluir_prefixos, date_time_id):
    ...
    cedec_stats_shp.to_file(f"./results/acumulado_24_cedec_{ontem.strftime('%Y-%m-%d')}.shp", driver="ESRI Shapefile")


    # Make upload to Geodados
    if upload_to_geonode:
        geodados.make_upload_to_geonode(f"acumulado_24_cedec_{str(ontem.strftime('%Y-%m-%d'))}", f"./results/acumulado_24_cedec_{str(ontem.strftime('%Y-%m-%d'))}.shp", {
    ...
    cedec_figure_path = f"./results/cedec_figure_{hoje_format}.png"
    plt.savefig(cedec_figure_path)    
    st.pyplot(fig)
    ...
    if maior_chuva_valor < 3:
        ...
        st.write(f"""
        AS Regionais de Proteção da Defesa Civil (REPDEC) com maior chuva no Estado de São Paulo, a partir da estatística "{estatistica_desejada}", foram **{maior_chuva_cedec}**, com um total de **{maior_chuva_valor:.2f} mm** de precipitação, seguido de **{maior_chuva_cedec2}**, com um total de **{maior_chuva_valor2:.2f} mm**, e **{maior_chuva_cedec3}**, com um total de **{maior_chuva_valor3:.2f} mm**.
        """)
